...
- `options` `{Object}`：适配器的配置对象。
  ...
  - `rateLimit` `{number}`：（可选）每个 socket 每秒允许接收的最大 WebSocket 消息数量，默认值为 `100`。

    **说明**：  
    限制单个 WebSocket 连接在 1 秒窗口内可以发送给服务器的消息数量（包括心跳消息）。超出限制的消息将在该窗口剩余时间内被静默丢弃；窗口结束后计数会自动重置。  
    此选项仅作为单连接的入站保护措施，**不会**限制业务消息监听器在广播到房间或其他 socket 时触发的 Redis 发布次数（例如通过 `broadcastToRoom`、`broadcast`、`emitCrossServer`）。Redis 发布层面的限流不在此选项范围内。  
    设置为 `0` 可完全禁用限流。

    ⚠️ **行为变更**：此选项默认值为 `100`，而非无限制。升级到启用此默认值的版本后，现有部署将自动获得每秒每 socket 100 条消息的上限，除非显式传入 `rateLimit: 0`。
